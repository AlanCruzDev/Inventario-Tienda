...
import {ProductoInterface,ProductoActualizar} from '../Interfaces/producto.interface';
...
export default class ProductoService{
  ...
  ActualizamosProducto=(req:Request,res:Response)=>{
    ...
    try{
      console.log(body);
      mysqlConnection.query(queryProductosTIenda.procedimientoActualizar,[body._NombreProducto,body._Precio,body._CodigoSerie,body._Imagen,body._fkProveedor,body._fkCategoria,body._Cantidad,body._FechaLllegada,body._fkTienda],
        (error: MysqlError | null, results: any, fields: any) =>{
          if(!error){
            res.json({
            ok:true
  ...
  ConsultaProducto= async(req:Request,res:Response)=>{
    const {id}=req.params;
    try{
      await mysqlConnection.query(queryProductosTIenda.ObtenerProducto,[id],
        (error: MysqlError | null, results: any[], fields: any)=>{
          if(!error){
              res.json({
                ok:true,
